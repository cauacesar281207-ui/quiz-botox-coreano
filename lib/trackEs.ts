...
const FUNNEL_SLUG = 'botox-coreano-es'
const SID_KEY = 'bcm_session_id_es'
...
export async function track(patch: {
  ...
  try {
    ...
    const prevAnswersRaw = localStorage.getItem('bcm_answers_es')
    const prevAnswers = prevAnswersRaw ? JSON.parse(prevAnswersRaw) : {}
    const mergedAnswers = { ...prevAnswers, ...(patch.answers ?? {}) }
    if (patch.answers) {
      localStorage.setItem('bcm_answers_es', JSON.stringify(mergedAnswers))
    }

    const prevStep = Number(localStorage.getItem('bcm_step_es') ?? 0)
    const nextStep = patch.step_reached != null ? Math.max(prevStep, patch.step_reached) : prevStep
    if (patch.step_reached != null) {
      localStorage.setItem('bcm_step_es', String(nextStep))
    }
